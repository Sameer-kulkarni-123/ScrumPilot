    ...
    def create_sprint(
        ...
        try:
            ...
            if not board_id:
                ...
                if not boards:
                    # Attempt to create a board automatically
                    logger.info(f"No board found for {project_key}, attempting to auto-create one.")
                    try:
                        # 1. Create Filter
                        filter_payload = {
                            "name": f"Filter for {project_key} Scrum Board",
                            "jql": f"project = {project_key} ORDER BY Rank ASC",
                            "description": "Auto-generated by ScrumPilot"
                        }
                        filter_res = self.client._session.post(f"{self.url}/rest/api/3/filter", json=filter_payload)
                        filter_res.raise_for_status()
                        filter_id = filter_res.json()["id"]
                        ...
                        board_res = self.client._session.post(f"{self.url}/rest/agile/1.0/board", json=board_payload)
                        board_res.raise_for_status()
                        board_id = board_res.json()["id"]
                        logger.info(f"Successfully created new board ID: {board_id}")
                    except Exception as e:
                        logger.error(f"Failed to auto-create board: {e}")
                        return {"success": False, "error": f"No boards found and failed to auto-create one: {e}"}
                else:
